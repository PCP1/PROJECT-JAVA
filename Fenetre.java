...
// SELECT seance.Date,seance.HeureDebut,cours.Nom_Cours,type_cours.Nom_TypeCours FROM `seance`INNER JOIN cours ON seance.ID_Cours=cours.ID_Cours INNER JOIN type_cours ON seance.ID_TypeCours=type_cours.ID_TypeCours INNER JOIN groupe WHERE groupe.ID_Groupe=5
...
//    this.setLocation(800,600); //Définis la position de la fenetre
    ...
    public void actionPerformed(ActionEvent arg0){
        String query="SELECT cours.Nom_Cours,type_cours.Nom_TypeCours,seance.Date,seance.HeureDebut FROM `seance`INNER JOIN cours ON seance.ID_Cours=cours.ID_Cours INNER JOIN type_cours ON seance.ID_TypeCours=type_cours.ID_TypeCours INNER JOIN groupe WHERE groupe.ID_Groupe="+groupe;
        new RoomTable(query);
        }
    ...
    items1.addActionListener(new ActionListener(){
        ...
        JTextField nom=new JTextField("",10);
         room= nom.getText();
        d.add( new JLabel ("Entrez nom salle")); 
        d.add(nom);
        d.add(b);   
        d.setSize(300,300);    
        d.setVisible(true);
