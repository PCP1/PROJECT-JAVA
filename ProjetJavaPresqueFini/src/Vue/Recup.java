...
//TYPE DE COURS TYPE DE COURS TYPE DE COURS TYPE DE COURS TYPE DE COURS 
  ...
  public static String RecupNomTypeCours(Connection connect,int seance)
  {
      ...
      try{
          ...
          while(result.next())
          {
              for(int i = 1; i <= resultMeta.getColumnCount();i++)
              {
              System.out.println(result.getString("Nom_TypeCours"));
              nomtypecours = result.getString("Nom_TypeCours");
              }
          }    
  ...
  //METHODE POUR RECUP ID TypeCours Depuis Nom
  public static int RecupIDTypeCoursNom(Connection connect,String nom)
  {
      int idtypecours = 0;
      try{
          Statement state = connect.createStatement();
          
          String query = "SELECT ID_TypeCours FROM type_cours WHERE Nom_TypeCours='"+nom+"'";
...
//SALLE SALLE SALLE SALLE SALLE SALLE SALLE SALLE SALLE SALLE SALLE SALLE SALLE 
  ...
  //METHODE POUR RECUP ID SALLE DEPUIS LE NOM
  public static int RecupIDSalleNom(Connection connect,String nom)
  {
      int idsalle = 0;
      try{
          Statement state = connect.createStatement();
          
          String query = "SELECT ID_Salle FROM salle WHERE Nom_Salle='"+nom+"'";
...
//GROUPE GROUPE GROUPE GROUPE GROUPE GROUPE GROUPE GROUPE GROUPE GROUPE GROUPE 
  ...
  //METHODE POUR RECUP ID GROUPE DEPUIS LE NOM
  public static int RecupIDGroupeNom(Connection connect,String nom)
  {
      int idgroupe = 0;
      try{
          Statement state = connect.createStatement();
          
          String query = "SELECT ID_Groupe FROM groupe WHERE Nom_Groupe='"+nom+"'";
  ...
  //RECUPERATION IDSEANCE
  public static int RecupIDSeance(Connection connect)
  {
      int idseance = 0;
      try{
          Statement state = connect.createStatement();
          
          String query = "SELECT ID_Seance FROM seance ORDER BY ID_Seance DESC LIMIT 1 ";
          ...
          while(result.next())
          {
              ...
              idseance = result.getInt("ID_Seance");
              }
          }    
