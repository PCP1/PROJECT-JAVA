...
public class DAOseance_salles extends DAO<Seance_Salles>{
    ...
    public boolean findsalle(int salle, int capacite, int heuredebut, String jour, int semaine) throws SQLException {
        Seance_Salles seance_salle =new Seance_Salles();
        
        try{
            ResultSet result =this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT * FROM seance_salles WHERE ID_Salle="+salle);
            ...
            if(result.next())
            {
               ...
               return true;
                ...
                ResultSet result2 =this.connect.createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT * FROM salle WHERE salle.ID_Salle="+salle+" AND salle.Capacite_Salle>="+capacite);
                ...
                if(!result2.next()){
                    ...
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY).executeQuery("SELECT * FROM seance WHERE Date='"+jour+"' AND HeureDebut="+heuredebut+" AND Semaine="+semaine);
